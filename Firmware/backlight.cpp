...
void force_bl_on(bool section_start) {
    if (section_start) {
        ...
        if (backlightLevel_HIGH < 30) {
            backlightLevel_HIGH = 30;
        }
    } else {
...
void backlight_update() {
    ...
    if (backlightMode == BACKLIGHT_MODE_AUTO) {
        if (backlightTimer.expired((uint32_t)backlightTimer_period * 1000ul)) {
            analogWrite(LCD_BL_PIN, backlightLevel_LOW);
        } else if (backlightTimer.running()) {
            analogWrite(LCD_BL_PIN, backlightLevel_HIGH);
        } else {/*do nothing*/;} //display is dimmed.
    } else if (backlightMode == BACKLIGHT_MODE_DIM){ 
        analogWrite(LCD_BL_PIN, backlightLevel_LOW);
    } else {
        analogWrite(LCD_BL_PIN, backlightLevel_HIGH);
    }
