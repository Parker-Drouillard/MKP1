...
#endif
  if (prusa_sd_card_upload) { //If an upload is actively happening
...
void process_commands() {
  ...
  if (code_seen("M117")) { //moved to highest priority place to be able to to print strings which includes "G", "PRUSA" and "^"
    ...
    #### Usage
         PRUSA [ Ping | PRN | thx | RESET | fv | M28 | SN | Fir | Rev | Lang | Lz | Beat | FR ]
...
//  ## ##     ##       ###  
    ...
    */
    ...
    case 76:  //PINDA probe temperature compensation calibration
      gcode_G76();
    break;
...
//	printf_P(_N("END M-CODE=%u\n"), mcode_in_progress);
  ...
  ####
  Modified 16 Nov 2020 - Parker Drouillard - Parker@pepcorp.ca
  Added new functionality. Automatically adjust for extruder offsets and set up for printing.
  */
  else if(code_seen('T')) {
    int index;
    for (index = 1; *(strchr_pointer + index) == ' ' || *(strchr_pointer + index) == '\t'; index++);
