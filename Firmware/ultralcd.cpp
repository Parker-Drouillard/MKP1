...
void lcd_wait_interact() {
  lcd_clear();
  lcd_set_cursor(0, 1);
  lcd_puts_P(_i("Please press knob"));////MSG_PRESS c=20 r=2
}
...
static void mainTest(){
	...
	MENU_ITEM_FUNCTION_P(PSTR("YES"), runPindaTest);
  ...
  typedef struct {
	...
	switch(_md->menuState) {
		...
		case 1: //normal menu structure.
		{
			...
			if (card.ToshibaFlashAir_isEnabled()){
				MENU_ITEM_FUNCTION_P(_T(MSG_REFRESH), lcd_sd_refresh); 
			}//show the refresh option if in flashAir mode.
#endif
			} else {
				MENU_ITEM_FUNCTION_P(PSTR(LCD_STR_FOLDER ".."), lcd_sd_updir);
			} //Show the updir button if in a subdir.
  	...
  	for (uint16_t i = 0; i < _md->fileCnt; i++) // Every file, from top to bottom.
			{
				if (menu_item == menu_line) //If the file is on the screen.
				{
					...
					MENU_ITEM_DUMMY();
				} //dummy item that just increments the internal menu counters.
			}
			MENU_END();
		} break;
